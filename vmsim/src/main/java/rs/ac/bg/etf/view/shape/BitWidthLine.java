...
/**
 * A straight schematic wire with an optional arrow tip at its end point and an optional bit-width
 * indicator (the diagonal "slash" tick plus an "Nb" label) sitting at its midpoint, matching the
 * signal-wire notation used across the MMU/TLB diagrams.
 *
 * <p>Everything is bindable: the four end-point coordinates, the bit count, the two toggles, and the
...
public class BitWidthLine extends Group {
    ...
    public BitWidthLine() {
        ...
        // The arrow head follows one toggle; the tick and label follow the other. Unmanaging the
        // hidden pieces keeps them out of this Group's bounds so nothing branches off a stale point.
        ...
        getChildren().addAll(wire, arrowHead, tick, bitsLabel);
        ...
        // The label can only be centred once it has been measured, so recompute when its size settles.
    ...
    private void updateGeometry() {
        ...
        // Stop the stroke at the arrow head's base so it never pokes through the tip.
        ...
        if (withArrow) {
            double baseX = x2 - ux * arrow;
            double baseY = y2 - uy * arrow;
